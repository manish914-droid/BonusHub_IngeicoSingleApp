...
class CreateSettlementPacket @Inject constructor(private var appDao: AppDao) :
    ISettlementPacketExchange {

    override fun createSettlementISOPacket(): IWriter = IsoDataWriter().apply {
        val batchListData = runBlocking(Dispatchers.IO) { appDao?.getBatchTableData() }
        ...
        if (batchListData?.isNotEmpty() == true) {
            if (tpt != null) {
                ...
                //adding field 61
                addFieldByHex(61, addPad(AppPreference.getString("serialNumber"), " ", 15, false) + AppPreference.getBankCode())
                ...
                addFieldByHex(62, ConnectionType.GPRS.code +
                        addPad(AppPreference.getString("deviceModel"), " ", 6, false)
