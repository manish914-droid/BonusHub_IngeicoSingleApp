...
import com.bonushub.crdb.utils.LYRA_IP_ADDRESS
import com.bonushub.crdb.utils.PORT2
            ...
            parseTableData(issuerParameterTable, splitter) {
                GlobalScope.launch(Dispatchers.IO) {
                    ...
                    when (issuerParameterTable.actionId) {
                        "1", "2" -> {
                            insertStatus =
                                appDatabase?.dao()?.insertIssuerDataInTable(issuerParameterTable)
                                    ?: 0L
                        }
                        "3" -> appDatabase?.dao()?.deleteIssuerParameterTable(issuerParameterTable)
                    }
            ...
            parseTableData(cardDataTable, splitter) {
                GlobalScope.launch(Dispatchers.IO) {
                    ...
                    when (cardDataTable.actionId) {
                        "1", "2" -> {
                            insertStatus =
                                appDatabase?.dao()?.insertCardDataInTable(cardDataTable) ?: 0L
                        }
                        "3" -> appDatabase?.dao()?.deleteCardDataTable(cardDataTable)
                    }
            ...
            parseTableData(hdfcTpt, splitter) {
                GlobalScope.launch(Dispatchers.IO) {
                    ...
                    when (hdfcTpt.actionId) {
                        "1", "2" -> {
                            insertStatus =
                                appDatabase?.dao()?.insertHDFCTPTDataInTable(hdfcTpt) ?: 0L
                        }
                        "3" -> appDatabase?.dao()?.deleteHDFCTPT(hdfcTpt)
                    }
            ...
            parseTableData(hdfcCdt, splitter) {
                GlobalScope.launch(Dispatchers.IO) {
                    ...
                    when (hdfcCdt.actionId) {
                        "1", "2" -> {
                            insertStatus = appDatabase?.dao()?.insertHDFCCDTInTable(hdfcCdt) ?: 0L
                        }
                        "3" -> appDatabase?.dao()?.deleteHDFCCDT(hdfcCdt)
                    }
...
fun getROC(): String? {
    return runBlocking(Dispatchers.IO) { appDatabase?.dao()?.getRoc() }
}

//region========================Reset ROC===============
fun resetRoc() =
    appDatabase?.dao()?.clearRoc(addPad(1, "0", 6, true), TableType.TERMINAL_PARAMETER_TABLE.code)
//endregion

//region========================Get Invoice=================
fun getInvoice(): String? {
    return runBlocking(Dispatchers.IO) { appDatabase?.dao()?.getInvoice() }
}
//endregion
...
fun resetInvoiceNumber() = appDatabase?.dao()
...
/*   val dao = TerminalParameterDao()
    ...
    fun getOtherInfo(): String {
        return "~${HDFCApplication.networkStrength}~${getBtry()}~${HDFCApplication.imeiNo}~${HDFCApplication.simNo}~${HDFCApplication.operatorName}"
    }
}
//endregion
...
fun getTctData(): TerminalCommunicationTable? {
    ...
    runBlocking(Dispatchers.IO) {
        tctData = appDatabase?.dao()?.getTerminalCommunicationTableData()?.get(0)
    }
    return tctData
}
//endregion
...
fun getTptData(): TerminalParameterTable? {
    ...
    runBlocking(Dispatchers.IO) {
        tptData = appDatabase?.dao()?.getAllTerminalParameterTableData()?.get(0)
    }
    return tptData
}
//endregion
...
fun getHDFCTptData(): HDFCTpt? {
    val hdfcTpt = runBlocking(Dispatchers.IO) { appDatabase?.dao()?.getAllHDFCTPTTableData() }
    return if (hdfcTpt?.size ?: 0 > 0)
        runBlocking(Dispatchers.IO) { appDatabase?.dao()?.getAllHDFCTPTTableData()?.get(0) }
    else
        null
}
//endregion
...
fun getCDTData(): TerminalCommunicationTable? {
    ...
    runBlocking(Dispatchers.IO) {
        cdtData = appDatabase?.dao()?.getTerminalCommunicationTableData()?.get(0)
    }
    return cdtData
}
...
fun getHDFCCDTData(): HDFCCdt? {
    val hdfcTpt = runBlocking(Dispatchers.IO) { appDatabase?.dao()?.getAllHDFCCDTTableData() }
    return if (hdfcTpt?.size ?: 0 > 0)
        runBlocking(Dispatchers.IO) { appDatabase?.dao()?.getAllHDFCCDTTableData()?.get(0) }
    else
        null
}
//endregion
...
fun getNII(): String {
    return runBlocking {
        appDatabase?.dao()?.getTerminalCommunicationTableData()?.get(0)?.nii ?: "0000"
    }
}
//endregion
...
fun checkForAccountTypeDialog(panNumber: String): Boolean {
    val hdfcCDTData = appDatabase?.dao()?.getCardDataByHDFCCDTPanNumber(panNumber)
...
//region=========================Below method to check HDFC CDT Fields Check:-
fun checkForTransactionAllowedOrNot(panNumber: String, transactionType: TransactionType): Boolean {
    val hdfcCDTData = appDatabase?.dao()?.getCardDataByHDFCCDTPanNumber(panNumber)
    Log.d("HDFC TPT:- ", hdfcCDTData.toString())
    var data: String? = null
    return if (hdfcCDTData != null) {
...
fun getBrandEMIMasterCategoryTimeStampsData(): Pair<String, String> {
    val data =
        runBlocking(Dispatchers.IO) { appDatabase?.dao()?.getAllBrandEMIMasterCategoryData() }
